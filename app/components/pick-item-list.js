...
export default class PickItemList extends React.PureComponent {
  ...
  loadRefreshData = (params = {}) => {
    ...
    Actions.getPickListByFilter(computedParams).then(data => {
      if (data.succeeded) {
        data.subSet = pageNumber === 1 ? data.subSet : [...this.state.list.subSet, ...data.subSet];
          this.setState({
          list: data,
          loading: false,
          refreshing: false,
  ...
  refresh = () => {
    this.setState({refreshing: true, loading: true, pageNumber: 1}, this.loadRefreshData);
  };
      ...
      <View style={styles.container} {...this.props}>
        ...
        ) : null}
        {
          ...
          ? <SelectList
              ...
              onEndReachedThreshold={this.state.hasNextPage ? parseInt((4 / this.state.list.subSet.length).toFixed(2)) : null}
              ...
              title={pickList => (
                <View style={styles.line}>
                  ...
                  <Text style={[styles.lineText, styles.lineTextTitle]}>{pickList.pickListId}</Text>

...
class Barcode extends React.PureComponent {
  ...
  setStateIfMounted = (obj, callback) => {
    if (this.mounted) this.setState(obj, callback);
  };
  ...
  inputTextChanged = barcode => {
    ...
    this.setStateIfMounted({animateTextStatus: false, oldScanTo: this.props.scanTo}, () => {
      this.submitScan();
    });
  };
  ...
  renderErrorModal() {
    ...
    AppDialog$.next({
      type: 'error',
      title: strings.MobileUI['Barcode.Alert.WrongBarcode.Title'] || 'Locale Error',
      content: strings.MobileUI[`Barcode.Alert.${this.state.error}.Body.${this.props.scanTo}`] ||
        strings.MobileUI[`Barcode.Alert.${this.state.error}.Body`] ||
        strings.MobileUI[`Barcode.Alert.WrongBarcode.Body.${this.props.scanTo}`] ||
  ...
  render() {
    ...
    return (
      <View style={[styles.wrapView]}>
        ...
        <IndicatorButton
          icon={<Icon name="camera" size={smallImage ? 35 : 50} color="#fffa"/>}
          text=""
          onPress={() => {
        ...
        />
        {
          this.state.cameraStatus ?
            <BarcodeScanner strings={strings} text={this.renderText()}
                            onScanBarcode={this.onScanBarcodeWithCamera.bind(this)}/> : null
        }
      </View>
    );
  }

...
const devMode = false;
const username = 'demo@haydigo.com';
const password = 'AGioVtps2';
...
class LoginView extends React.PureComponent {
  ...
  login = () => {
    ...
    Actions.login({username: this.state.username, password: this.state.password})
      .then(data => {
  ...
  async changeServer() {
    ...
    await AsyncStorage.setItem('environmentUrl', environmentUrl === STAGING_URL ? PRODUCTION_URL : STAGING_URL);
    await AsyncStorage.removeItem('accessToken');
    AppDialog$.next({
      type: 'info',
      title: 'change server',
      content: `connect to ${environmentUrl === STAGING_URL ? 'production' : 'staging'}`,
      expiredTime: 10 * 1000,
      status: true
    });
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={[styles.card, (this.state.focus ? styles.cardAbsolute : null)]}>
          <TextField
            label={
              strings

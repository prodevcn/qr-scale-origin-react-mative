...
class DialogWrapper extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.expiredTime) {
      ...
      this.timeout = setTimeout(this.props.onPressBackdrop, this.props.expiredTime);
    }
  }
  ...
  renderContent(childrenWithProps) {
    return <View style={[StyleSheet.absoluteFill, styles.dialogContainer]}>
      <Dialog
        onPress={this.props.withoutTouchable ? null : this.props.onPressModalContent || this.props.onPressBackdrop}
        style={{
          container: [
            styles.dialog
        ...
        {
          this.props.icon ? <Icon style={[styles.icon]} name={this.props.icon} size={21} color="#fff" /> : null
        }
        {
          childrenWithProps
        }
  ...
  render() {
    ...
    const childrenWithProps = React.Children.map(children,
      child => React.cloneElement(child, {type: this.props.type, icon: this.props.icon, rawContent: this.props.rawContent})
    );
...
class Title extends React.Component {
  render() {
    return (
      <Dialog.Title
        style={{titleText: [styles.titleText, (this.props.type ? styles[this.props.type + 'TitleText'] : undefined)]}}>
        {
          this.props.children
        }
      </Dialog.Title>
    ...
    return (
      <Dialog.Content style={{
      ...
      }}>
        {
         ...
         : (
             <Text style={[styles.contentText, (this.props.type ? styles[this.props.type + 'ContentText'] : undefined)]}>
               {
                 this.props.children
               }
             </Text>

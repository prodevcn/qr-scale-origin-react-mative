...
class ProductView extends React.PureComponent {
  ...
  static prepareProduct(product, {entityAttribute, ...props}) {
    const {identifier, title, barCode, image} = entityAttribute.wmsEntityAttributeDto;
  ...
  getAttrLabels = () => {
    ...
    const {identifier, title, barCode} = this.props.entityAttribute.wmsEntityAttributeDto;
  ...
  getImages() {
    ...
    return preparedProduct.imageValue && preparedProduct.imageValue.split(',') || [];
  }

  render() {
    const {strings, pickedQuantity, totalQuantity, showImage, storedAndOrdered, storedAndOrderedText} = this.props;

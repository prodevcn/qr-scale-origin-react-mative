...
class OrderProcess extends StackScreen {
  ...
  goToDetail = ({lineId}) => {
    ...
    this.props.navigation.navigate('OrderLineProcess', {orderLine, purchaseOrderId});
  };
  ...
  combineProducts(products) {
    ...
    return products.reduce((items, item) => {
      ...
      if (combinedProduct) {
        ...
        combinedProduct.bgColor = combinedProduct.ordered < combinedProduct.stored
          ? 'red' : combinedProduct.ordered === combinedProduct.stored ? 'green' : null;
      } else {
        item.bgColor = item.ordered < item.stored ? 'red' : item.ordered === item.stored ? 'green' : null;
        items.push(item);
      }
      return items;
    }, []);
  ...
  renderFinishModal() {
    ...
    >
      <DialogWrapper.Title>
        {strings.MobileUI[`Purchase.OrderProcess.Success.Title`] || 'Locale Error'}
      </DialogWrapper.Title>
      <DialogWrapper.Content>
        {strings.MobileUI[`Purchase.OrderProcess.Success.Body`] || 'Locale Error'}
      </DialogWrapper.Content>
      <DialogWrapper.Actions>
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`Purchase.OrderProcess.Finished.Ok`] || 'Locale Error'
          }
        ...
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`Purchase.OrderProcess.Finished.Cancel`] || 'Locale Error'
          }
          onPress={() => {
            this.setState({
              finishModalStatus: false
    ...
    >
      ...
      <DialogWrapper.Actions>
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`Purchase.OrderProcess.Success.OrderList`] || 'Locale Error'
          }

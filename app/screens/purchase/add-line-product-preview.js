...
class AddLineProductPreview extends StackScreen {
  ...
  goToLineProcess = () => {
    this.setState({loading: false, redirectModalStatus: false}, () => {
      this.props.navigation.navigate('OrderLineProcess', {orderLine: this.state.orderLine});
    });
  };
  goToOrderDetail = () => {
    this.setState({loading: false, redirectModalStatus: false}, () => {
      this.props.navigation.navigate('OrderProcess', {purchaseOrderId: this.state.purchaseOrderId});
    });
  };
  goToNewLine = () => {
    this.setState({loading: false, redirectModalStatus: false}, () => {
      this.props.navigation.navigate('AddLineScanProduct', {purchaseOrderId: this.state.purchaseOrderId});
    });
  };
  ...
  addOrderLine = () => {
    ...
    const {attributeList, dateTimeOptions, numberOptions, warehouseId} = this.props;
  ...
  renderRedirectModal() {
    ...
    >
      <DialogWrapper.Title>
        {strings.MobileUI[`Purchase.AddOrderLine.Success.Title`] || 'Locale Error'}
      </DialogWrapper.Title>
      <DialogWrapper.Content>
        {strings.MobileUI[`Purchase.AddOrderLine.Success.Body`] || 'Locale Error'}
      </DialogWrapper.Content>
      <DialogWrapper.Actions>
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`Purchase.AddOrderLine.Success.NewLine`] || 'Locale Error'
          }
        ...
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`Purchase.AddOrderLine.Success.OrderDetail`] || 'Locale Error'
          }
  ...
  render() {
    ...
    if (isSaving) {
      return <View style={{flex: 1, backgroundColor: '#333'}}><PageIndicator /></View>;
    }
        ...
        <View style={styles.container}>
          ...
          <View style={styles.actionsContainer}>
            <IndicatorButton
              ...
              onPress={parseInt(this.state.ordered) > 0 ? this.addOrderLine : () => {
              }}
              loading={isSaving}
            />
          </View>
...
const getWarehouseId = (home) => {
  ...
  return !warehouseId && warehouseList && warehouseList.length ? warehouseList[0].id : warehouseId;
};
...
export default connect(mapStateToProps)(withNavigationFocus(AddLineProductPreview));

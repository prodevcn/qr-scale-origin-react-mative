...
class OrderLineProcess extends StackScreen {
  ...
  locationScanned = (code, location) => {
    ...
    const selectedLocation = foundLocations.find(l => l.locationCode === code);
    ...
    Actions.purchaseOrderAddStockInput({orderLine, locationId, quantity}).then(
      data => {
        if (data.succeeded) {
          ...
          if (this.state.orderLine.foundLocations.find(item => item.locationId === locationId)) {
            ...
            this.state.orderLine.foundLocations.push(this.state.selectedLocation)
          }
          this.setState({selectedLocation: null, quantity: '0', isSaving: false, orderLine});
          if (orderLine.stored > orderLine.ordered) {
            AppDialog$.next({
              type: 'warning',
              title: strings.MobileUI['Purchase.OrderLineProcess.Alert.storedMoreThanOrdered.title'] || 'Locale Error',
              content: strings.MobileUI['Purchase.OrderLineProcess.Alert.storedMoreThanOrdered.body'] || 'Locale Error',
              expiredTime: 10 * 1000,
              status: true
            });
          }
  ...
  render() {
    ...
    const scannedText = this.props.strings.MobileUI['Purchase.OrderLineProcess.ScannedText'] || 'Locale Error';
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.wrapper}>
            <ProductView product={orderLine.product} showImage
                         storedAndOrderedText={scannedText}
                         storedAndOrdered={this.state.orderLine.stored + ' / ' + this.state.orderLine.ordered}/>
            <LocationView

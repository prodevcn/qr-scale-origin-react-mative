...
import {ActionSheetIOS, Alert, Platform, SafeAreaView, StatusBar, StyleSheet, View} from 'react-native';
...
class PickingStep extends StackScreen {
  ...
  startProcessing = () => {
    if (
      (this.pickList && this.pickList.status !== 'Processing')
    ) {
  ...
  loadNextStep = () => {
    ...
    const {selectedCart, selectedTray, errorReason, pickedQuantity} = this.state;
    ...
    if (currentStep) {
      if (errorReason) {
        step = {
          ...
          selectedLocationId: selectedLocation ? selectedLocation.locationId : currentStep.locationId ? currentStep.locationId : null,
          cartId: selectedCart ? selectedCart.id : currentStep.cartId ? currentStep.cartId : null,
          cartTrayId: selectedTray ? selectedTray.id : currentStep.cartTrayId ? currentStep.cartTrayId : null,
          cartTrayCode: selectedTray ? selectedTray.barcode : currentStep.cartTrayCode ? currentStep.cartTrayCode : null,
          continueWithError,
        ...
        step = {
          ...currentStep,
          pickedQuantity: !this.getEnabledBatchConfirm() ? currentStep.batchPickableQuantity : 1,
      ...
      if (data.succeeded) {
        if (data.result) {
          ...
          const expectedTray = this.cart
            ? this.props.cartTrayList.find(t => t.id === data.result.cartTrayId)
            : null;
  ...
  trayScanned = code => {
    ...
    this.setState({
      ...
      Actions.checkCartTrayAvailability(selectedTray.id, pickListId).then((data) => {
        if (data.errors && data.errors.length > 0) {
          const {strings} = this.props;
          const errorContent = errorsToContent(data.errors, strings);
          AppDialog$.next({
  ...
  inError = () => {
    this.setState({loading: true}, () => {
      getReasonMessages().then(errorReasonTypeList => {
        ...
        ActionSheetIOS.showActionSheetWithOptions(
          {
            options: [
              ...
              ...this.state.errorReasonTypeList.map(
                reason =>
                  reason.code
              )
            ],
  ...
  renderAndroidActionsSheet() {
    ...
    return <ActionSheet
      ...
      options={[
        ...
        strings.MobileUI['OrderPicking.PickStep.Alert.ProductError.Buttons.Cancel'] || 'Locale Error',
      ]}
  ...
  renderUpdateUserModal() {
    ...
    >
      <DialogWrapper.Title>
        {strings.MobileUI[`OrderPicking.PickStep.UserModal.Title`] || 'Locale Error'}
      </DialogWrapper.Title>
      <DialogWrapper.Content>
        {strings.MobileUI[`OrderPicking.PickStep.UserModal.Body`] || 'Locale Error'}
      </DialogWrapper.Content>
      <DialogWrapper.Actions>
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`OrderPicking.PickStep.UserModal.Button.Yes`] || 'Locale Error'
          }
          onPress={() => {
            this.setState({
              updateUserModalStatus: false,
        ...
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`OrderPicking.PickStep.UserModal.Button.No`] || 'Locale Error'
          }
          onPress={() => {
            this.setState({
              updateUserModalStatus: false,
  ...
  renderReasonContinueModal() {
    ...
    >
      <DialogWrapper.Title>
        {strings.MobileUI[`OrderPicking.PickStep.ContinueStep.Modal.Title`] || 'title'}
      </DialogWrapper.Title>
      <DialogWrapper.Content>
        {strings.MobileUI[`OrderPicking.PickStep.ContinueStep.Modal.Body`] || 'body'}
      </DialogWrapper.Content>
      <DialogWrapper.Actions>
        <IndicatorButton
          white
          raised
          text={
            strings.MobileUI[`OrderPicking.PickStep.ContinueStep.Modal.No`] || 'no'
          }
          onPress={() => {
            this.setState({
              reasonContinueModalStatus: false,
        ...
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`OrderPicking.PickStep.ContinueStep.Modal.Yes`] || 'yes'
          }
          onPress={() => {
            this.setState({
              reasonContinueModalStatus: false,
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        {
          Platform.OS === 'android'
            ? this.renderAndroidActionsSheet()
            : null
        }
        {
        ...
        {
          this.state.reasonContinueModalStatus ? this.renderReasonContinueModal() : null
        }
      </SafeAreaView>
    );
  }

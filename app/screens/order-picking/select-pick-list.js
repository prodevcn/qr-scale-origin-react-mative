...
class SelectPickList extends StackScreen {
  ...
  willScreenFocus = () => {
    if (!this.state.loading) this.refresh();
  };
  ...
  saveFilter = filter =>
    new Promise((resolve, reject) => {
      if (filter.id < 0) filter.id = 0;

      Actions.filterSave(filter).then(data => {
        if (data.succeeded) {
          const index = this.state.filters.findIndex(f => f.id === filter.id);
  ...
  deleteFilter = filterId =>
    new Promise((resolve, reject) => {
      Actions.filterDelete(filterId).then(data => {
        if (data.succeeded) {
      ...
      <SafeAreaView style={styles.container}>
        ...
        <DrawerLayout
          ...
          renderNavigationView={() => (
            <FilterTabsSettings
              filter={filters ? filters.find(f => f.id === editFilterId) : null}

...
class ScanCart extends StackScreen {
  ...
  barcodeScanned = (barcode, cart) => {
    ...
    if (this.pickList && cart.size < this.pickList.pickListCount) {
      AppDialog$.next({
        type: 'error',
        title: strings.MobileUI['BatchPicking.ScanCart.Alerts.CartSize.Title'] || 'Locale Error',
        content: strings.MobileUI['BatchPicking.ScanCart.Alerts.CartSize.Body'] || 'Locale Error',
        expiredTime: 10 * 1000,
        status: true
      });
      return;
    ...
    if (cart.isBeingUsed) {
      if (cart.cartUsingEntityType === 'Picking') {
        ...
        AppDialog$.next({
          type: 'error',
          title: strings.MobileUI['BatchPicking.ScanCart.Alerts.IsBeingUsed.Title'] || 'Locale Error',
          content: strings.MobileUI['BatchPicking.ScanCart.Alerts.IsBeingUsed.Body'] || 'Locale Error',
          expiredTime: 10 * 1000,
          status: true
        });
      }
  ...
  removeSos() {
    Actions.removePickListFromBatchPicking(this.pickList.id, this.state.SOs).then((res) => {
      if (res.succeeded) {
        this.startBatchPicking();
      } else if (res.errors && res.errors.length) {
        this.setState({loading: false}, () => {
  ...
  renderRemoveSOsModal() {
    ...
    >
      <DialogWrapper.Title>
        {strings.MobileUI[`BatchPicking.ScanCart.Alerts.XCartSize.Title`] || 'Locale Error'}
      </DialogWrapper.Title>
      <DialogWrapper.Content>
        <ScrollView style={{maxHeight: 50, width: '100%'}}>
          <Text style={{color: '#fff', fontSize: 16}}>
            {this.state.SOs.reduce((text, item, index) => {
              return text + (strings.MobileUI[item] || item) + (index < this.state.SOs.length - 1 ? ', ' : '');
            }, '')}
          </Text>
        </ScrollView>
      </DialogWrapper.Content>
      <DialogWrapper.Actions>
        <View style={{
          ...
          <IndicatorButton
            ...
            raised
            text={
              strings.MobileUI[`BatchPicking.ScanCart.Alerts.XCartSize.Continue`] || 'Locale Error'
            }
            onPress={() => {
              this.setState({loading: true, removeSOsModalStatus: false}, () => {
                this.removeSos();
              });
            }}
          ...
          <IndicatorButton
            ...
            raised
            text={
              strings.MobileUI[`BatchPicking.ScanCart.Alerts.XCartSize.Cancel`] || 'Locale Error'
            }
            onPress={() => {

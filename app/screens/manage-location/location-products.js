...
class LocationProducts extends StackScreen {
  ...
  loadRefreshData = () => {
    ...
    Actions.getProductsByLocationFilter(locationCode, pageNumber, pageSize).then(data => {
      if (data.succeeded) {
  ...
  barcodeScanned = (code) => {
    this.setState({loading: true}, () => {
      Actions.getProductByCodeAndLocationId(this.props.navigation.getParam('location').locationId, code).then(data => {
        let product = null;
        let redirectModalStatus = !data.result;
        if (data.result) {
          product = {...data.result.productDto, stockQuantity: data.result.stock ? data.result.stock.quantity : 0};
  ...
  renderRedirectModal() {
    ...
    >
      <DialogWrapper.Title>
        {strings.MobileUI[`ManageLocation.LocationProducts.Modal.Title`] || 'title'}
      </DialogWrapper.Title>
      <DialogWrapper.Content>
        {strings.MobileUI[`ManageLocation.LocationProducts.Modal.Body`] || 'body'}
      </DialogWrapper.Content>
      <DialogWrapper.Actions>
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`ManageLocation.LocationProducts.Modal.No`] || 'no'
          }
          onPress={() => {
        ...
        <IndicatorButton
          ...
          raised
          text={
            strings.MobileUI[`ManageLocation.LocationProducts.Modal.Yes`] || 'yes'
          }
          onPress={() => {
            this.setState({redirectModalStatus: false}, () => {
        ...
        <View style={styles.wrapper}>
          ...
          <View style={styles.middle}>
            <View style={styles.middleTextWrap}>
              <Text style={styles.middleText}>
                {strings.MobileUI['ManageLocation.ProductLocations.Fields.Or'] ||
                'Locale Error'}
              </Text>
            </View>
          ...
          <View style={styles.right}>
            ...
            <ProductList
              ...
              onEndReached={this.state.hasNextPage ? this.loadNextPage.bind(this) : null}
              onEndReachedThreshold={this.state.hasNextPage ? parseInt((2 / this.state.data.length).toFixed(2)) : null}

...
import {ActionSheetIOS, Platform, SafeAreaView, StatusBar, StyleSheet, View} from 'react-native';
...
class StepProcess extends StackScreen {
  ...
  loadNextStep = withoutCurrentStep => () => {
    const {currentStep, selectedLocation, modifyBatchPickableQuantity, quantity} = this.state;
    ...
    if (!withoutCurrentStep && currentStep) {
      if (errorReason) {
        ...
        const pickQuantity = (modifyBatchPickableQuantity !== '' ? parseInt(modifyBatchPickableQuantity) : currentStep.totalQuantity);
        step = {
          ...currentStep,
          selectedLocationId: selectedLocation.locationId,
          quantity: currentStep.stepType === stepTypes.pickPlacement ? (!this.getEnabledBatchConfirm()
            ? pickQuantity : 1) : parseInt(quantity),
          cartId: this.cart.id
        };
      }
  ...
  render() {
    const {strings} = this.props;
    const {loading, currentStep, completed, failed, modifyBatchPickableQuantity} = this.state;
            ...
            <View style={{flex: 1}}>
              ...
              <View style={styles.container}>
                    ...
                    <View>
                      ...
                      />
                      {
                        ...
                        ? <IndicatorButton
                            upperCase={false}
                            text={
                              (modifyBatchPickableQuantity || currentStep.totalQuantity).toString()
                            }
                            ...
                            onPress={() => {
                              if (this.drawer.openDrawer) {
                                this.setState({
                                  modifyBatchPickableQuantity: modifyBatchPickableQuantity !== ''
                                    ? modifyBatchPickableQuantity : currentStep.totalQuantity.toString()
                ...
                <View style={styles.wrapper}>
                  ...
                  />
                  {currentStep && currentStep.stepType === stepTypes.pickPlacement ? (
                      <IndicatorButton
                        upperCase={false}
                        text={

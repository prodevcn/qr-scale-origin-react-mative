...
import {DEVELOPMENT_URL, PRODUCTION_URL, STAGING_URL, TEST_URL} from './constants/apis';
...
// export function getCurrentEnvirontment() {
...
export async function getEnvironmentUrl() {
  ...
  return await AsyncStorage.getItem('environmentUrl') || environmentUrl.production;
}
...
export const errorsToContent = (errors, strings) => {
  return errors.reduce((error, item) => {
    let title = strings.ServiceError ? strings.ServiceError[item.code] : item.code;
